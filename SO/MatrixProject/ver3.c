...
void compute_cell(int cords[2]) {	
	...
	for(int i=0; i < matrix_length; i++) {
		int hyb = (i+y) % matrix_length;

		val2 = matrix2[y][hyb];

		pthread_mutex_lock(&matrix_res_lock[x][hyb]);
		matrix_res[x][hyb] += val1 * val2;
		pthread_mutex_unlock(&matrix_res_lock[x][hyb]);
	}
...
{	
	...
	/* Setup Heap Allocated array of mutexes */
	...
	/* --- Initial Benchmarking checkpoint (COMPUTATION STARTS HERE) --- */
	...
	/* --- Final Benchmarking checkpoint (COMPUTATION ENDS HERE) --- */
